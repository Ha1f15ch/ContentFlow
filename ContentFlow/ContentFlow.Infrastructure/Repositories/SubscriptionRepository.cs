    ...
    public async Task<List<SubscriptionWithFollowingProfileDto>> GetListSubscriptionFollowersByFollowerAsync(int follower, CancellationToken ct = default)
    {
        return await _context.Subscriptions
            ...
            .Join(
                ...
                (subscription, userProfile) => new { subscription, userProfile })
            .Join(
                _context.Users,
                row => row.userProfile.UserId,
                user => user.Id,
                (row, user) => new SubscriptionWithFollowingProfileDto
                {
                    SubscriptionId = row.subscription.Id,
                    FollowingProfileId = row.userProfile.Id,
                    FollowingUserName = user.UserName,
                    FollowingAvatarUrl = row.userProfile.AvatarUrl,
                    IsPaused = row.subscription.IsPaused,
                    IsActive = row.subscription.IsActive,
                    SubscribedAt = row.subscription.CreatedAt
                })
            .ToListAsync(ct);
    }
    ...
    public async Task<List<SubscriptionWithFollowerProfileDto>> GetListSubscriptionFollowingAsync(int followingId, CancellationToken ct = default)
    {
        return await _context.Subscriptions
            ...
            .Join(
                ...
                row => row.userProfile.UserId,
                user => user.Id,
                (row, user) => new SubscriptionWithFollowerProfileDto
                {
                    SubscriptionId = row.subscription.Id,
                    FollowerProfileId = row.userProfile.Id,
                    FollowerUserName = user.UserName,
                    FollowerAvatarUrl = row.userProfile.AvatarUrl,
                    IsPaused = row.subscription.IsPaused,
                    IsActive = row.subscription.IsActive,
                    SubscribedAt = row.subscription.CreatedAt
                })
